...
def demo2():
    root = Tk() 
    root.option_add("*font" , "impact 25")
    Label(root , text = "mocha"  ,bg = "red" ).pack(side = LEFT , fill = X , ipadx = 20 ,ipady = 40 , padx = 20 , pady = 40)
    Label(root , text = "latte" , bg = "green" ).pack(side = LEFT , fill = X , ipadx = 20 ,ipady = 30 ,padx = 40)
    Label(root , text = "espresso"  , bg = "blue").pack(side = LEFT , fill = X , ipadx = 20)
    Button()
    root.mainloop()
...
def demo3 ():
    ...
    for i,m in enumerate(menu):
        Label(root , text = m ,bg = color[i] , width = 15 ).pack(side = LEFT , fill = X , ipadx = 20 ,ipady = 40 , padx = 10 , pady = 10)
